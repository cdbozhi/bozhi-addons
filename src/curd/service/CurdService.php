...
class CurdService
{
     ...
     * @var string
     */
    protected $tablePrefix = 'bozhi_';
    /**
     * 数据库名
     * @var string
     */
    protected $database = 'bozhiadmin';
    ...
    public function __construct()
    {
        ...
        $this->tplPath = $this->rootPath . 'vendor' . DS . 'bozhiadmin'. DS . 'bozhi-addons' . DS . 'src' . DS . 'curd'  . DS . 'tpl' . DS ;
        return $this;
    }
    /**
     * 获取配置
...
//                $v = explode(',',$v[0]);
    ...
    protected function setArg()
    {
        ...
        $this->module = $this->config['module'] ?: 'admin';
        ...
        foreach ($methodArr as $k => $v) {
            if ($v != 'refresh') {
                ...
                if(!in_array($v,['restore'])) {
                    $controllerPrefix  = $this->addon?"addons/$this->addon/". ($this->module=='common'?'admin':$this->module) ."/":"";
                    $space = $k==0?'':'                    ';
        ...
        if (!$this->addon) {
            ...
            $this->fileList = [
                ...
                'controllerFrontFileName' => $this->rootPath . "addons" . DS . "{$this->addon}" . DS . "index" . DS . "controller" . DS . ($controllerArr ? Str::lower($controllerArr[0]) . DS . $this->controllerName . '.php' : $this->controllerName . '.php'),
        ...
        if($this->addon){
            $controllerTplFront = str_replace(
                [
                    ...
                    '{{$limit}}'],
                [
                    str_replace('admin', $this->module, $this->controllerNamespace),
                    $this->controllerName,
            ...
            $this->makeFile(
                str_replace(
                    'admin',
                    $this->module=='admin'?"index":$this->module,
    ...
    protected function makeMenu(int $type=1)
    {
        ...
        $childMenu =  [
            'href' => $this->addon?'addons/'.$this->addon.'/admin/'.lcfirst($this->controllerName):($this->controllerArr ? strtolower($this->controllerArr[0]) . '.' . lcfirst($this->controllerName) : lcfirst($this->controllerName)),
            'title' => $title,
            'status' => 1,
            'menu_status' => 1,
            'type' => 1,
        ...
        foreach (explode(',', $this->method) as $k => $v) {
            ...
            if ($this->addon) {
                $menuList[] = [
                    'href'=>'addons/'.$this->addon.'/admin/' . lcfirst($this->controllerName . '/' . $v),
                    'title'=>$v,
                    'status'=>1,
                    'menu_status'=>0,
                    'icon'=>'layui-icon layui-icon-app'
                ];
                $childMethod[] = 'addons/'.$this->addon.'/admin/' . lcfirst($this->controllerName . '/' . $v);
            } else {
                $menuList[] = [
        ...
        $parentMethod = $this->addon?'addons/'.$this->addon.'/admin/'.lcfirst($this->controllerName):($this->controllerArr ? strtolower($this->controllerArr[0]) . '.' . lcfirst($this->controllerName) : lcfirst($this->controllerName));
