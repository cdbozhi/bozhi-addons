...
class Menu extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if (!$param['addon']) {
            $class = 'app\\admin\\controller' . $nameSpace . '\\' . $this->controllerName;
        } else {
            $class = 'addons\\' . $this->addon . '\\admin\\controller' . $nameSpace . '\\' . $this->controllerName;
        }
        try {
            if (class_exists($class)) {
                ...
                foreach ($methods as $m) {
                    ...
                    if ($this->addon) {
                        $menuList[] = [
                            'href' => 'addons/' . $this->addon . '/admin/' . lcfirst($this->controllerName . '/' . $m->getName()),
                            'title' => trim($title),
                            'status' => 1,
                            'menu_status' => 0,
