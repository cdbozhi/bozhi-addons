...
use bozhi\addons\middleware\Addons;
use bozhi\addons\Service;
use bozhi\helper\FileHelper;
use think\Exception;
use think\facade\Db;
...
\think\Console::starting(function (\think\Console $console) {
    $console->addCommands([
        'addons:config' => '\\bozhi\\addons\\command\\SendConfig',
        'auth:config' => '\\bozhi\\auth\\command\\SendConfig',
        'curd:config' => '\\bozhi\\auth\\command\\SendConfig'
    ]);
});
...
if (!function_exists('get_addons_autoload_config')) {
    ...
    function get_addons_autoload_config($chunk = false)
    {
        ...
        $route = [];
        // 读取插件目录及钩子列表
        $base = get_class_methods("\\bozhi\\Addons");
